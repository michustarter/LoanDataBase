...
FROM employee 
WHERE (YEAR(NOW())-YEAR(birth_date) - (DATE_FORMAT(NOW(), '%m%d') < DATE_FORMAT(birth_date, '%m%d'))>25); 
...
SELECT first_name, last_name 
FROM employee e 
WHERE e.last_name LIKE '_F%';

-- d
SELECT brand, model, max(mileage) 
FROM car;

-- e
SELECT COUNT(*) 
FROM car c 
WHERE c.mileage> 200000 and c.mileage<300000;
...
-- g-1
SELECT customer_id, first_name, last_name, COUNT(*) loans_number 
FROM customer c 
LEFT JOIN loan l
...
-- g-2
SELECT MONTH(rental_date) month, COUNT(*) loans_number 
FROM loan 
GROUP BY month
ORDER BY month;
...
-- g-4
SELECT MONTH(rental_date) month, round(COUNT(*)/(SELECT COUNT(DISTINCT customer_id) 
FROM loan l),2) as average 
FROM loan 
GROUP BY month ASC; 

-- h
SELECT first_name, last_name, COUNT(DISTINCT car_id) rented_cars 
FROM customer c 
LEFT JOIN  loan l 
ON l.customer_id=c.id 
GROUP BY customer_id 
ORDER BY rented_cars desc 
LIMIT 3;
...
-- j
SELECT first_name, last_name, SUM(loan_price) final_price 
FROM customer cr 
LEFT JOIN loan ln 
...
-- n
SELECT SUBSTRING_INDEX(customer.mail,'@',-1) domain, COUNT(*) amount 
FROM customer
GROUP BY domain 
ORDER BY domain 
LIMIT 1;
...
FROM car c 
WHERE EXISTS (SELECT k.employee_id 
FROM keeper k 
WHERE c.id=k.car_keeped_id 
HAVING COUNT(DISTINCT k.employee_id) >= 2); 
 
-- p
SELECT e.id employee_id, e.first_name,e.last_name 
FROM employee e
LEFT JOIN keeper k 
ON k.employee_id = e.id 
